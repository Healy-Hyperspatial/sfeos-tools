...
class STACClient:
    ...
    def get_item(self, collection_id: str, item_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.client.get(f"{self.base_url}/collections/{collection_id}/items/{item_id}")
            response.raise_for_status()
            return response.json()
        except Exception as e:
...
def get_thumbnail_url(item: Dict[str, Any]) -> Optional[str]:
    ...
    for asset_key, asset_data in assets.items():
        if 'thumbnail' in asset_data.get('roles', []) or 'overview' in asset_data.get('roles', []):
            return asset_data['href']
    
    # Fallback to any image asset
    for asset_key, asset_data in assets.items():
        asset_type = asset_data.get('type', '').lower()
        if any(img_type in asset_type for img_type in ['image/jpeg', 'image/png', 'image/jpg']):
            return asset_data['href']
...
def create_map(items: List[Dict[str, Any]]) -> Any:
    ...
    for item in items:
        ...
        if geom['type'] == 'Point':
            ...
            polygon_coords = [[[coord[1], coord[0]] for coord in ring] for ring in coords]
            folium.Polygon(
...
def run_viewer(base_url: str):
    ...
    with st.sidebar:
        ...
        collection_names = ["All Collections"] + [c.get('id', 'Unknown') for c in collections]
    ...
    with col1:
        ...
        if search_clicked or 'items' not in st.session_state:
            with st.spinner("Searching items..."):
                collection_id = None if selected_collection == "All Collections" else selected_collection
    ...
    with col2:
        ...
        if items:
            # Item selector
            item_ids = [item.get('id', f'Item {i}') for i, item in enumerate(items[:50])]
            ...
            st.dataframe(item_data, use_container_width=True, height=300)
        ...
        if selected_item:
            ...
            if assets:
                ...
                if selected_asset_key:
                    ...
                    st.markdown(f"**Roles:** {', '.join(asset['roles']) if asset['roles'] else 'N/A'}")
                    
                    # Display image if it's an image asset
                    asset_type = asset['type'].lower()
                    if any(img_type in asset_type for img_type in ['image/', 'jpeg', 'png', 'jpg', 'tif']):
                        with st.spinner("Loading image..."):
                            img = load_image_from_url(asset['href'])
                            if img:
                                st.image(img, caption=asset['title'], use_container_width=True)
                            else:
            ...
            if thumbnail_url and not assets:
                with st.spinner("Loading thumbnail..."):
                    ...
                    if img:
                        st.image(img, caption="Thumbnail", use_container_width=True)
        else:
